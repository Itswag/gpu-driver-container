...
dep_installer () {
  ...
  # Download unzboot as kernel images are compressed in the zboot format on RHEL-compatible 9 arm64
...
extra_pkgs_install() {
  if [ "$DRIVER_TYPE" != "vgpu" ]; then
      ...
      # If running on a community RHEL-compatible base image, enable the CRB repository.
      OS_RELEASE_ID=$(grep -oP '(?<=^ID=).+' /etc/os-release | tr -d '"')
      if [ "$OS_RELEASE_ID" = "rocky" ] || [ "$OS_RELEASE_ID" = "almalinux" ]; then
